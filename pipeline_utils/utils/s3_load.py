...
from pipeline_utils.utils.ls_cfg import LS_CFG
...
s3 = boto3.client(service_name='s3',
                  aws_access_key_id=LS_CFG.aws_access_key_id,
                  aws_secret_access_key=LS_CFG.aws_secret_access_key,
                  region_name=LS_CFG.region_name
)

bucket_name = LS_CFG.import_bucket_name
logger = setup_logger()
...
def create_s3_directory(bucket_name, directory_name):
    ...
    try:
        ...
        try:
            ...
            logger.error("Error occur when creating s3 directory", exc_info=True)
...
def upload_json_to_s3(file_dir,json_data,bucket=None):
    ...
    try:
        ...
        logger.error("Failed to upload json to S3", exc_info=True)
...
def upload_img_to_s3(directory_name,img_data,img_name,bucket=None):
    ...
    try:
        ...
        logger.info(f"Image successfully uploaded to {img_path} in bucket {bucket_name}.")

    except Exception as error:
        logger.error("Failed to upload image to S3", exc_info=True)
